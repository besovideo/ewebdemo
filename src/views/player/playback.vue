<template>
  <div>
    <div class="playbackMainDiv">
      ...
      <div style="height: calc(100vh - 400px); width: calc(100vw - 200px); position: absolute; left: 100px; overflow-x: auto;">
        <table cellspacing="0px" v-show="recordFiles.length > 0" border="1px" style="margin: 0 auto;">
          <tr align="center">
            <td>puID</td>
            <td>fileType</td>
            <td>fileName</td>
    ...
    <el-dialog v-model="dialogVisible" draggable destroy-on-close title="视频回放" align-center @closed="onDialogClosed">
      ...
      <!-- 
       ...
       -->
      <div style="width: 100%;height: 400px;" ref="refPlaybackDiv">
...
<script>
import { ref } from "vue"
import { PlayBack } from "@besovideo/webrtc-player";
    ...
    async playFiles(fileid) {
      ...
      try {
        ...
        const { instance } = PlayBack({
          type: "auto",
          puOption: this.puid ? {
            puId: this.puid, //填写了这个字段就是设备端回放
            //puId: 'PU_20084838'
            //id: ""
          } : undefined,
          container: this.$refs.refPlaybackDiv,
          token: this.token,
          fileID: this.mediaUrl,
          zIndexMenu: 10,
        })

        this.playbackInstance = instance;
        // 打开
        this.playbackInstance.open()
